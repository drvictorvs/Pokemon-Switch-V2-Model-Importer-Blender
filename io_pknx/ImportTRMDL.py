...
def from_trmdl_scvi(filep: str, trmdl: BufferedReader, settings: dict):
  ...
  if settings["loadlods"] == False:
    trmsh_count = 1

  result = read_trmsh_scvi(filep, trmsh_lods_array, trmsh_count)

  if result is not None:
    trmsh, trmbf, poly_group_count = result
  else:
...
def read_trmtr_scvi(filep: str, trmtr: BufferedReader) -> Tuple[list[Material], list[dict]]:
  materials = []
...
def read_trskl_scvi(
  ...
  # LINE 1247
  if settings["basearmature"] == "loadbasearm":
...
def from_trmdl_pla(filep: str, trmdl: BufferedReader, settings: dict):
  ...
  materials, mat_data_array = read_trmtr_pla(filep, trmtr, chara_check)
...
def read_trskl_pla(trmdl: BufferedReader, new_collection: Collection, trskl: BufferedReader):
  bone_array = []
  bone_id_map = {}
  bone_rig_array = []
  trskl_bone_adjust = 0
