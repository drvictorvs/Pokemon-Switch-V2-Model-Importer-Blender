      ...
      for x in range(bone_count):
        ...
        if trskl_bone_struct_ptr_bone != 0:
          ...
          if IN_BLENDER_ENV:
            ...
            if bonestructh == True:
              if trskl_bone_struct_ptr_h == 0 & bpy.app.version < (4,1,0):
...
def register():
    ...
    if get_current_menu_item(bpy.types.TOPBAR_MT_file_import, ImportTRMDL_menu_func_import) is None:
        bpy.types.TOPBAR_MT_file_export.append(ImportTRMDL_menu_func_import)
    else:
        func = get_current_menu_item(bpy.types.TOPBAR_MT_file_import, ImportTRMDL_menu_func_import)
...
def unregister():
    ...
    if get_current_menu_item(bpy.types.TOPBAR_MT_file_import, ImportTRMDL_menu_func_import) is not None:
        bpy.types.TOPBAR_MT_file_export.remove(ImportTRMDL_menu_func_import)

if __name__ == "__main__":
  register()
  
def get_current_menu_item_by_name(menu, name):
    for func in menu._dyn_ui_initialize():
