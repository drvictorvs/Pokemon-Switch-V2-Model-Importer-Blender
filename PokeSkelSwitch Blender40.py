bl_info = {
    "name": "Pokémon Switch V2 WIP (.trskl)",
...
class PokeSVSkelImport(bpy.types.Operator, ImportHelper):
    bl_idname = "custom_import_scene.pokeskelscarletviolet"
    bl_label = "Import"
    ...
    def execute(self, context):
        ...
        if self.multiple == False:
            ...
            from_trsklsv(directory, f, self.rare, self.loadlods, self.bonestructh)
            f.close()
            return {'FINISHED'}  
        else:
...
def from_trsklsv(filep, trskl, rare, loadlods, bonestructh):
    # make collection
    if IN_BLENDER_ENV:
...
def from_trskl(filep, trskl, rare, loadlods):
    # make collection
    if IN_BLENDER_ENV:
...
def menu_func_import(self, context):
    self.layout.operator(PokeArcSkelImport.bl_idname, text="Legends Arceus (.trskl)")
    self.layout.operator(PokeSVSkelImport.bl_idname, text="Scarlet Violet (.trskl)")
